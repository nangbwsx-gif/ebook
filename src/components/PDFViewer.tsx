...
export default function PDFViewer({ pdfUrl, bookId }: PDFViewerProps) {
  ...
  const [displayScale, setDisplayScale] = useState(1.5) // 即时反馈的显示缩放
  ...
  const [hasTallPage, setHasTallPage] = useState(false)
  ...
  // 渲染缓存 key=scale → ImageData
  const zoomCacheRef = useRef<Map<string, ImageData>>(new Map())
  // 当前展示的缩放对应的 cache key
  const currentZoomKeyRef = useRef<string>('1.5')
  // 防抖定时器
    ...
    async function loadPDF() {
      try {
        ...
        for (let p = 1; p <= doc.numPages; p++) {
          ...
          if (page.getViewport({ scale: 1 }).height > TALL_PAGE_THRESHOLD) { tall = true; break }
        }
        setHasTallPage(tall)
        if (tall && doc.numPages === 1) setScrollMode(true)
  ...
  useEffect(() => {
    ...
    async function doRender() {
      ...
      // 检查缓存
      const cached = zoomCacheRef.current.get(cacheKey)
      if (cached) {
  ...
  const renderToCanvas = useCallback(async (pageNum: number, canvas: HTMLCanvasElement): Promise<boolean> => {
    ...
    try {
      ...
      if (cached) { canvas.width = cached.width; canvas.height = cached.height; ctx.putImageData(cached, 0, 0); return true }
  ...
  const preloadNeighbors = useCallback(async (pageNum: number) => {
    ...
    for (const p of neighbors) {
      ...
      try { const offscreen = document.createElement('canvas'); await renderToCanvas(p, offscreen) } catch { /* ok */ }
  ...
  useEffect(() => {
    ...
    let cancelled = false
    const isZoom = (displayScale !== scale)
    setLoading(true)
    setShowJump(false)
    renderToCanvas(currentPage, mainCanvasRef.current).then((ok) => {
      if (!cancelled) { if (ok) { setLoading(false); preloadNeighbors(currentPage) } else setError('页面渲染失败') }
    }).catch(() => { if (!cancelled) setError('页面渲染出错') })
    return () => { cancelled = true }
  }, [pdfDoc, currentPage, renderToCanvas, preloadNeighbors, scrollMode, scale])

  useEffect(() => {
    if (prevLoadingRef.current && !loading && scrollRef.current && !scrollMode) {
      const el = scrollRef.current
      el.style.transition = 'none'; el.style.opacity = '0.95'; el.style.transform = 'translateY(5px)'
  ...
  // 翻页模式缩放防抖：CSS 即时缩放 + 后台重新渲染
  const pageRerenderTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  ...
  const zoomIn  = () => {
    if (scrollMode) {
      ...
      const next = Math.min(3, Math.round((displayScale + 0.25) * 100) / 100)
      setDisplayScale(next)
      if (pageRerenderTimerRef.current) clearTimeout(pageRerenderTimerRef.current)
      pageRerenderTimerRef.current = setTimeout(() => setScale(next), 350)
    }
  }
  const zoomOut = () => {
    if (scrollMode) {
      ...
      const next = Math.max(0.5, Math.round((displayScale - 0.25) * 100) / 100)
      setDisplayScale(next)
      if (pageRerenderTimerRef.current) clearTimeout(pageRerenderTimerRef.current)
      pageRerenderTimerRef.current = setTimeout(() => setScale(next), 350)
    }
  ...
  const resetView = () => {
    if (scrollMode) setScale(1.5)
    else { setCurrentPage(1); setDisplayScale(1.5); setScale(1.5); if (pageRerenderTimerRef.current) clearTimeout(pageRerenderTimerRef.current) }
  }

  // 重绘完成后同步 displayScale
  useEffect(() => {
    if (!scrollMode && !loading && scale > 0.3) setDisplayScale(scale)
  }, [loading, scrollMode, scale])

  const handleCanvasClick = (e: React.MouseEvent) => {
    if (scrollMode || touchSwipedRef.current) { touchSwipedRef.current = false; return }
    if (turning) return
  ...
  const onTouchStart = (e: React.TouchEvent) => { touchStartX.current = e.touches[0].clientX; touchStartY.current = e.touches[0].clientY; touchSwipedRef.current = false }
  const onTouchMove = (e: React.TouchEvent) => { if (Math.abs(e.touches[0].clientX - touchStartX.current) > 10 || Math.abs(e.touches[0].clientY - touchStartY.current) > 10) touchSwipedRef.current = true }
  ...
  const handleJump = (e: React.FormEvent) => { e.preventDefault(); const p = parseInt(jumpInput); if (p >= 1 && p <= totalPages) { setCurrentPage(p); setShowJump(false); setJumpInput('') } }
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center max-w-sm px-6">
          ...
          <button onClick={() => window.location.reload()} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded-lg transition">重新加载</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col h-full select-none">
      <header className="bg-gray-900/90 backdrop-blur-sm border-b border-gray-800/50 px-3 py-1.5 flex items-center justify-between shrink-0 gap-1 z-30">
        {!scrollMode ? (
          <div className="flex items-center gap-0.5">
            <button onClick={goPrev} disabled={currentPage <= 1 || turning} className="p-2 text-gray-400 hover:text-white hover:bg-white/10 rounded-lg disabled:opacity-20 active:scale-90">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>
            </button>
            <div className="relative mx-0.5">
              <button onClick={() => { setShowJump(true); setTimeout(() => jumpInputRef.current?.focus(), 50) }} className="px-2.5 py-1.5 text-sm text-gray-300 hover:text-white hover:bg-white/10 rounded-lg font-medium tabular-nums min-w-[80px] text-center">
                <span className="text-white">{currentPage}</span><span className="text-gray-600 mx-0.5">/</span><span className="text-gray-500">{totalPages}</span>
              </button>
              {showJump && (
                <form onSubmit={handleJump} className="absolute top-full left-1/2 -translate-x-1/2 mt-2 bg-gray-800 border border-gray-700 rounded-xl p-2 shadow-2xl z-50 flex items-center gap-1.5"
                  onKeyDown={e => { if (e.key === 'Escape') { setShowJump(false); setJumpInput('') } }}>
                  <input ref={jumpInputRef} type="number" min={1} max={totalPages} value={jumpInput} onChange={e => setJumpInput(e.target.value)}
                    className="w-18 px-2 py-1.5 bg-gray-900 border border-gray-600 rounded-lg text-white text-sm text-center focus:outline-none focus:border-blue-500 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none" placeholder="页数" autoFocus />
            ...
            <button onClick={goNext} disabled={currentPage >= totalPages || turning} className="p-2 text-gray-400 hover:text-white hover:bg-white/10 rounded-lg disabled:opacity-20 active:scale-90">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-300">
            ...
            <span>滚动浏览</span><span className="text-gray-600 ml-1">{scrollProgress}%</span>
            {loading && <span className="text-yellow-500 text-xs ml-2">高清渲染中...</span>}
          </div>
        )}
        ...
        {!scrollMode && (
          <div className="hidden sm:flex items-center flex-1 max-w-xs mx-3">
            <div className="w-full h-1 bg-gray-800 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-blue-600 to-blue-400 rounded-full transition-all duration-300 ease-out" style={{ width: `${progress}%` }} />
            </div>
          </div>
        )}

        <div className="flex items-center gap-0.5">
          <button onClick={zoomOut} disabled={scale <= 0.5} className="p-2 text-gray-400 hover:text-white hover:bg-white/10 rounded-lg active:scale-90 disabled:opacity-20" title="缩小">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M20 12H4" /></svg>
          </button>
          <span className="text-xs text-gray-400 w-12 text-center tabular-nums font-medium">
            {Math.round((scrollMode ? scale : displayScale) * 100)}%
          </span>
          <button onClick={zoomIn} disabled={scale >= 3} className="p-2 text-gray-400 hover:text-white hover:bg-white/10 rounded-lg active:scale-90 disabled:opacity-20" title="放大">
          ...
          {hasTallPage && (
            <>
              ...
              <button onClick={() => { setScrollMode(s => !s); setCurrentPage(1) }}
                className={`px-2.5 py-1 text-xs rounded-lg transition flex items-center gap-1 ${scrollMode ? 'bg-blue-600/20 text-blue-400' : 'text-gray-400 hover:text-white hover:bg-white/10'}`}
          ...
          <div className="w-px h-5 bg-gray-700/50 mx-0.5" />
          <button onClick={resetView} className="p-2 text-gray-400 hover:text-white hover:bg-white/10 rounded-lg active:scale-90" title="重置">
      ...
      {hasTallPage && !scrollMode && (
        <div className="bg-amber-900/20 border-b border-amber-800/20 px-3 py-1 text-center">
          <span className="text-xs text-amber-300">
            ...
            <button onClick={() => setScrollMode(true)} className="ml-2 underline hover:text-amber-200">试试滚动模式</button>
          </span>
        </div>
      )}
            ...
            {loading && (
              <div className="flex items-center justify-center h-64">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <span className="text-xs text-gray-500">高清加载中...</span>
                </div>
              </div>
            )}
            <canvas ref={scrollCanvasRef} className="shadow-2xl rounded-sm"
              style={{ display: loading ? 'none' : 'block', width: '100%', height: 'auto' }} />
          </div>
        ) : (
          <div ref={zoomContainerRef} className="relative my-4 inline-flex group"
            style={scrollMode ? undefined : { zoom: scale > 0 ? displayScale / scale : 1 }}>
            {loading && !turning && (
        ...
        <div className="flex items-center gap-3 text-[11px] text-gray-600">
          {scrollMode ? (
            <>
              ...
              <span className="w-px h-3 bg-gray-800" /><span>上下滑屏浏览</span>
            </>
          ) : (
            <>
              <span className="flex items-center gap-1">
              ...
              <span className="w-px h-3 bg-gray-800" /><span>点击左右翻页</span>
              <span className="w-px h-3 bg-gray-800" /><span>滑动翻页</span>
            </>
          )}
        </div>
      </div>
